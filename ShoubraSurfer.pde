PImage BKG,Charfrwd,Charleft,Charright,GameOver,Opening;
...
int Random(){                  //Random Coin spawn lane
...
void draw(){
if(int(startTimer.getTime())>15)     // Open Screen Section
{
  image(Opening,0,0);
    textFont(font2);
textSize(50);
...
text("Press ENTER To start",25,285,800,800);
 ...
 startTimer.countDown();
  ...
  if(int(startTimer.getTime())<=15 && startTimer.getTime()>=0){        // Play Section
  background(90);
...
if(startTimer.getTime()<=0)   // End screen
{
  image(GameOver,0,0);
  textFont(font2);
textSize(80);
